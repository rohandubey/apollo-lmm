...
sys.path.append(os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/"))
...
def init():
    ...
    model_path = os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/")
...
def run(raw_data):
    ...
    download_from_azure_file(share_name, "humm_input", file_path, os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/in", file_path), account_name, account_key)
    infer(0, os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/in", file_path), os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/out", "gen_" + file_path), os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/models/GuitarRock.pth"),  os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/models/index/added_IVF3498_Flat_nprobe_1_GuitarRock_v2.index"), "cuda:0", "rmvpe")
    # another pipeline without watermark.
    # add_watermark_interval(os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/out", "gen_" + file_path))
    upload_to_azure_file(os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/out", "gen_" + file_path), share_name, "humm_output", "gen_" + file_path, account_name, account_key)
...
def add_watermark_interval(input_output_audio_path, start_time_ms=2000, interval_ms=4000, watermark_volume_reduction=12):
    download_from_azure_file(share_name, "FMM_inputs", 'DhunVO.wav', os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/out", 'DhunVO.wav'), account_name, account_key)
    watermark_audio_path = os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Inference/out", 'DhunVO.wav')
