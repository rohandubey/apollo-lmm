...
def run(raw_data):
    ...
    infer(0, os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Indian/in", file_path), os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Indian/out", "gen_" + file_path), os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Indian/models/Tablaa.pth"),  os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Indian/models/index/added_IVF1536_Flat_nprobe_1_tabla_model_v2.index"), "cuda:0", "rmvpe")
    # another pipeline for demucs
    # overlay_demucs(os.path.join(os.getenv("AZUREML_MODEL_DIR"), "RVC-CLI-Indian/out", "gen_" + file_path))
...
def overlay_demucs(input_path):
    # Load the separator model and separate the audio file
    separator = demucs.api.Separator(model="htdemucs")
    origin, separated = separator.separate_audio_file(input_path)

    # Remove vocals and save the audio
    overlayed_audio = separated['drums'] + separated['bass'] + separated['other']
    demucs.api.save_audio(overlayed_audio, input_path, samplerate=separator.samplerate)
